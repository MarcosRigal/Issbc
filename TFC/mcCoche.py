...
class Coche(Clase):
    ...
    def __init__(self, nombre):
        '''
        @param nombre: Nombre de la Coche
        '''
...
class Mercedes_AMG_SL(Coche):
    '''
    El objeto es una  Coche .

    '''

    def __init__(self):
        # Se inicia con el nombre naranja
...
if __name__ == '__main__':
    ...
    while cont == 's':
        ...
        if ej == 1:  # Crea la lista de atributos que va a usar la base de conocimiento
            atributos = creaAtributosBC(
                [('atAS', 'int', 'mm'), ('atLS', 'int', 'mm'), ('atAP', 'int', 'mm'), ('atLP', 'int', 'mm')])
            print(atributos)
            for at in atributos:
        ...
        if ej == 25:  # Crear un conjunto de caracter�sticas
            ...
            print(c1.atributo.nombre, c1.atributo.tipo,
                  c1.atributo.unidad, c1.valor)
            pass
        if ej == 3:  # Crea un objeto pasando su identificador y los valores de los atributos

            lct = [[Atributo('atAS', 'int', 'mm'), 30], [Atributo(
                'atAP', 'int', 'mm'), 30], [Atributo('atLP', 'int', 'mm'), 45]]
            print(lct)
            # Se crean instancias de la lista de atributos
            llct = creaCaracteristicas(lct)
            # se crea un objeto a partir de las instancias de la lista de atributos
            ob = Objeto('p1', llct)
        ...
        if ej == 5:
            ...
            cMercedes_AMG_SL = Mercedes_AMG_SL()
            print('Descripcion de Mercedes_AMG_SL',
                  cMercedes_AMG_SL.descripcion())
            lct = [[Atributo('Marca', 'str', None), 'Mercedes'], [Atributo('Carroceria', 'str', 'None'), 'Deportivo'], [
                Atributo('Plazas', 'int', 'nº'), 2], [Atributo('Potencia', 'int', 'cv'), 575]]
            print(lct)
            # Se crean instancias de la lista de atributos
            llct = creaCaracteristicas(lct)
            ...
            rb = buscaReglaComparableEnUnaClase(
                ob.caracteristicas[2], cMercedes_AMG_SL)
